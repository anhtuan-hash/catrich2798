...
Recommended identity validation:

- `VITE_CHATWOOT_IDENTITY_ENDPOINT=/api/chatwoot-identity`
- `CHATWOOT_HMAC_SECRET`: the HMAC token from the Chatwoot Website inbox identity-validation settings
- `SUPABASE_URL`
- `SUPABASE_ANON_KEY`

`CHATWOOT_HMAC_SECRET` is server-only and must never use the `VITE_` prefix.
