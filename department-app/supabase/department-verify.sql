...
with expected_department as (
  ...
  select
    ...
    exists (
      select 1 from storage.buckets where id = 'department-files' and public = false
...
join expected_department e on e.id = d.id
where lower(dm.email) = lower('anhtuanpek@gmail.com');
