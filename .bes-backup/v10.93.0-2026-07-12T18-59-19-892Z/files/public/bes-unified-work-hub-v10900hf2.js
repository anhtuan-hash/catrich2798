(function bootstrapBESUnifiedWorkHub() {
  ...
  function isSupabaseClient(candidate) {
    return !!(candidate && typeof candidate.from === 'function' && typeof candidate.rpc === 'function' && candidate.auth && typeof candidate.auth.getSession === 'function');
  }
  ...
  function discoverClient() {
    var candidates = [
      ...
      window.supabase,
      window.__SUPABASE__,
      window.BESSupabaseBridge && window.BESSupabaseBridge.client
    ];
  ...
  function clientAdapter(client) {
    ...
    function callRpc(name, body) {
      if (!client || typeof client.rpc !== 'function') {
        throw new Error('Supabase runtime chưa cung cấp rpc(). Hãy tải lại sau khi Runtime Bridge sẵn sàng.');
      }
      return client.rpc(name, body || {});
    }
    return {
      mode: 'client',
      client: client,
      async context() { return unwrap(await callRpc('work_hub_my_context')); },
      async people() { return unwrap(await callRpc('work_hub_people')) || []; },
      async dashboard() { return unwrap(await callRpc('work_hub_dashboard')) || {}; },
      async items() {
      ...
      async transition(id, nextStatus, note) {
        return unwrap(await callRpc('work_hub_transition_item', {
          target_item: id,
  ...
  function installObservers() {
    ...
    window.addEventListener('bes-supabase-bridge-ready', function () {
      state.adapter = null;
      state.adapterError = '';
      if (state.open) loadAll(false);
    });
