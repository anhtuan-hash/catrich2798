...
function fail(message) {
  console.error(`\n[V10.87.1 rollback] ${message}\n`);
  process.exit(1);
}

if (!fs.existsSync(statePath)) fail('Không tìm thấy trạng thái cài đặt .bes-release/v10.87.1.json.');
const state = JSON.parse(fs.readFileSync(statePath, 'utf8'));
const backupDir = path.resolve(cwd, state.backupDir || '');
if (!state.backupDir || !fs.existsSync(backupDir)) fail('Không tìm thấy thư mục backup của V10.87.1.');
...
for (const asset of [
  'public/bes-command-center-v10871.css',
  'public/bes-command-center-v10871.js',
  'public/bes-release-v10.87.1.json'
]) {
...
fs.rmSync(statePath);
console.log('\nĐã rollback V10.87.1 về trạng thái trước khi cài đặt.');
console.log(`Đã dùng backup: ${state.backupDir}`);
console.log('Dữ liệu Launcher cá nhân trong localStorage không bị xóa.');
