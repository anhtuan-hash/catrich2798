(function bootstrapBrianCommandCenter() {
  ...
  var VERSION = '10.87.1';
  var SCHEMA = 1;
  var BASE_KEY = 'bes-command-center-v10870'; // preserve V10.87.0 user configuration
  var CATALOG_KEY = 'bes-command-center-catalog-v10870'; // preserve discovered routes
  var CHANNEL_NAME = 'bes-command-center-v10870'; // keep cross-tab compatibility
  var MAX_RECENT = 18;
  ...
  function toneForItem(item, pref) {
    var group = normalizeText((pref && pref.group) || item.group || '');
    if (/tai nguyen|library|resource/.test(group)) return 'resource';
    if (/giang day|teaching|lesson|activity/.test(group)) return 'teaching';
    if (/ky nang|skill|reading|speaking|listening|writing/.test(group)) return 'skill';
    if (/kiem tra|assessment|exam|test/.test(group)) return 'assessment';
    if (/chuyen mon|department|hoc sinh|student/.test(group)) return 'department';
    if (/quan tri|admin|system health|ai governance/.test(group)) return 'admin';
    if (/tien ich|utility|tool/.test(group)) return 'utility';
    return 'system';
  }

  function cardHtml(item) {
    var pref = getPref(item);
    var tone = toneForItem(item, pref);
    return '<article class="bescc-card" data-bescc-tone="' + tone + '" data-bescc-item="' + escapeHtml(item.id) + '">' +
