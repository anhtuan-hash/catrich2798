(function bootstrapBESSupabaseRuntimeBridge(){
  'use strict';
  if(window.BESSupabaseBridge&&window.BESSupabaseBridge.version==='10.90.0-HF2')return;

  var VERSION='10.90.0-HF2';
  var CACHE_KEY='bes-supabase-public-config-v10900hf2';
  ...
  async function rpcRequest(functionName,params){
    try{
      var ready=await ensureReady();
      if(ready.native&&typeof ready.native.rpc==='function'){
        return await ready.native.rpc(functionName,params||{});
      }
      var url=ready.config.url.replace(/\/$/,'')+'/rest/v1/rpc/'+encodeURIComponent(functionName);
      var headers={
        apikey:ready.config.key||ready.session.access_token,
        Authorization:'Bearer '+ready.session.access_token,
        'Content-Type':'application/json'
      };
      var response=await fetch(url,{method:'POST',headers:headers,body:JSON.stringify(params||{})});
      var raw=await response.text(),data=raw?safeParse(raw,raw):null;
      if(!response.ok){
        var errorData=data&&typeof data==='object'?data:{};
        return {data:null,error:{
          message:errorData.message||errorData.details||String(data||response.statusText),
