...
import { summarizeAttendance, compareGradeWindows, groupGradeComparisons } from '../../src/studentSupport/studentSupportAnalytics.js';
...
test('grade comparisons are separated by subject', () => {
  const rows = [
    { date: '2026-09-01', score: 8, subject: 'English' },
    { date: '2026-09-02', score: 7, subject: 'English' },
    { date: '2026-09-03', score: 6, subject: 'English' },
    ...
    { date: '2026-09-11', score: 5, subject: 'English' },
    { date: '2026-09-12', score: 4, subject: 'English' },
    { date: '2026-09-01', score: 9, subject: 'Math' },
  ];
  const grouped = groupGradeComparisons(rows, 3);
  assert.deepEqual(grouped.map((item) => item.subject), ['English', 'Math']);
  assert.equal(grouped[0].comparison.delta, -2);
  assert.equal(grouped[1].comparison.hasEnoughData, false);
});
