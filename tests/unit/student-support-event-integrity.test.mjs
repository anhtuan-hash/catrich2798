...
test('case event policy binds events to the real case and limits teaching-team writes', () => {
  ...
  assert.match(sql, /c\.id\s*=\s*student_support_case_events\.case_id/i);
  assert.match(sql, /c\.student_ref\s*=\s*student_support_case_events\.student_ref/i);
  assert.match(sql, /c\.homeroom_workspace_id\s*=\s*student_support_case_events\.homeroom_workspace_id/i);
  ...
  assert.match(sql, /n\.id::text\s*=\s*coalesce\(metadata\s*->>\s*'note_id'/i);
});
