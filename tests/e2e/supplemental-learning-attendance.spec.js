...
const css=[
  ...
  fs.readFileSync(path.join(root,'src/styles/SupplementalClasses.css'),'utf8'),
].join('\n');
...
test.describe('Supplemental learning attendance UI',()=>{
  ...
  test('Supplemental class workspace exposes class, teacher and roster controls',async({page})=>{
    await page.setViewportSize({width:1280,height:800});
    await page.setContent(`<section class="bes-supplemental-dialog bes-supplemental-admin-workspace"><header class="bes-supplemental-header"><div><span class="bes-supplemental-source-badge">ĐIỂM DANH</span><h2>Lớp học bổ sung</h2><p>Tạo lớp, quản lý giáo viên và học sinh thủ công, sau đó điểm danh như các lớp khác.</p></div><button>×</button></header><main class="bes-supplemental-main"><section class="bes-supplemental-class-home"><div class="bes-supplemental-class-toolbar"><label>Tìm lớp<input type="search" placeholder="Tên lớp, môn, giáo viên, học sinh"></label><button class="is-primary">+ Tạo lớp học bổ sung</button></div><div class="bes-supplemental-class-grid"><article class="bes-supplemental-class-card"><header><div><span class="bes-supplemental-source-badge">LỚP HỌC BỔ SUNG</span><h3>Bổ sung Toán 12</h3><p>Toán · Khối 12</p></div><span class="bes-supplemental-status">Đang hoạt động</span></header><div class="bes-supplemental-class-meta"><span><b>Phòng</b>P.12</span><span><b>Giáo viên</b>Trần Nguyên Dự</span><span><b>Học sinh</b>2 đang học</span></div><footer><button class="is-primary">Quản lý</button><button>Điểm danh</button><button>Lịch sử</button><button class="is-danger-quiet">Xóa lớp</button></footer></article></div></section></main></section>`);
    await installCss(page);
    await expect(page.getByRole('heading',{name:'Lớp học bổ sung'})).toBeVisible();
    await expect(page.getByPlaceholder('Tên lớp, môn, giáo viên, học sinh')).toBeVisible();
    for(const label of ['Tạo lớp học bổ sung','Quản lý','Điểm danh','Lịch sử','Xóa lớp'])await expect(page.getByRole('button',{name:new RegExp(label)})).toBeVisible();
    await expect(page.getByText('Trần Nguyên Dự')).toBeVisible();
    await expect(page.getByText('2 đang học')).toBeVisible();
