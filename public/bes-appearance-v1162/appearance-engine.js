const BES_APPEARANCE_VERSION = '11.6.2';
...
function enhanceLegacyAppearanceCard() {
  if (!/settings|cai-dat|appearance/i.test(routeKey())) return;
...
function decorateLegacyControls(scope = document) {
  const advanced = [...scope.querySelectorAll('button,a,div,span')].find((el) => textMatches(el, ['thiết lập giao diện nâng cao', 'advanced appearance']));
  if (advanced && !advanced.dataset.besBound) {
    advanced.dataset.besBound = 'true';
    advanced.style.cursor = 'pointer';
    advanced.addEventListener('click', (event) => { event.preventDefault(); openStudio('theme'); });
  }
  const mappings = [
  ...
  [...scope.querySelectorAll('button,[role="button"]')].forEach((button) => {
    ...
    const mapping = mappings.find((item) => textMatches(button, item.labels));
    if (!mapping) return;
...
function setupObservers() {
  observer = new MutationObserver(() => scheduleLegacyEnhancement());
