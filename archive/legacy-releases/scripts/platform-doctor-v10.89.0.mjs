...
const build = readJson(path.join(cwd, 'public', 'bes-platform-build-v10.89.0.json'));
const migrations = readJson(path.join(cwd, 'public', 'bes-migrations-v10.89.0.json'));
const modules = readJson(path.join(cwd, 'public', 'bes-modules-v10.88.0.json'));
const flags = readJson(path.join(cwd, 'public', 'bes-feature-flags-v10.88.0.json'));

if (!pkg) finding('error', 'PACKAGE_INVALID', 'package.json không tồn tại hoặc không hợp lệ.');
if (pkg && pkg.version !== '10.89.0') finding('error', 'VERSION_PACKAGE', 'package.json chưa ở V10.89.0.', pkg.version);
if (!version || version.version !== '10.89.0') finding('error', 'VERSION_MANIFEST', 'public/version.json chưa đồng bộ V10.89.0.', version?.version || 'missing');
if (!build || build.version !== '10.89.0') finding('error', 'BUILD_MANIFEST', 'Build manifest V10.89.0 chưa hợp lệ.');
if (!modules || !Array.isArray(modules.modules)) finding('error', 'MODULE_REGISTRY', 'Module Registry không hợp lệ.');
if (!flags || !Array.isArray(flags.flags)) finding('error', 'FEATURE_FLAGS', 'Feature Flags không hợp lệ.');
if (!fs.existsSync(path.join(cwd, 'supabase', 'work_hub_v10_89_0.sql'))) finding('error', 'WORK_HUB_SQL', 'Thiếu migration Work Hub.');

const sqlFiles = walk(path.join(cwd, 'supabase')).filter(({ file }) => file.endsWith('.sql'));
if (!migrations) finding('error', 'MIGRATION_MANIFEST', 'Không đọc được migration inventory V10.89.0.');
...
if (migrations && migrations.requiredMigration !== 'work_hub_v10_89_0.sql') finding('warning', 'REQUIRED_MIGRATION', 'Migration bắt buộc chưa được đánh dấu đúng.', migrations.requiredMigration || 'missing');
...
if (cssPatches.length > 20) finding('warning', 'CSS_PATCH_COUNT', 'Có nhiều lớp CSS runtime; nên hợp nhất trong bản tái cấu trúc tiếp theo.', String(cssPatches.length));
else finding('info', 'CSS_PATCH_COUNT', 'Số lớp CSS runtime trong ngưỡng theo dõi.', String(cssPatches.length));

const scanTargets = [...sourceFiles, ...publicFiles.filter(({ file, stat }) => stat.size < 600_000 && /\.(js|json|html|css)$/.test(file))];
...
if (modules?.modules) {
  ...
  if (!ids.includes('work-hub')) finding('error', 'WORK_HUB_MODULE', 'Module Registry chưa có work-hub.');
...
if (!flags?.flags?.some((flag) => flag.key === 'work_hub.enabled')) {
  finding('error', 'WORK_HUB_FLAG', 'Feature Flags chưa có work_hub.enabled.');
}
...
const reportPath = path.join(reportDir, 'platform-doctor-v10.89.0.json');
...
console.log(`\nPlatform Doctor V10.89.0: ${summary.totals.error} lỗi · ${summary.totals.warning} cảnh báo · ${summary.totals.info} thông tin.`);
