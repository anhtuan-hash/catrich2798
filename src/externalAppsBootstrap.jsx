...
function Bootstrap(){const[user,setUser]=useState(null),[language,setLanguage]=useState(()=>localStorage.getItem('bet-language')||'vi');useEffect(()=>{let active=true;initializeAuthSession().then(u=>active&&setUser(u)).catch(()=>{});const un=subscribeToAuthChanges(u=>active&&setUser(u));const o=new MutationObserver(()=>setLanguage(document.documentElement.lang==='en'?'en':'vi'));o.observe(document.documentElement,{attributes:true,attributeFilter:['lang','data-language']});return()=>{active=false;un?.();o.disconnect();};},[]);return <><ExternalAppsIntegration currentUser={user} language={language}/><RandomStudentPickerBridge currentUser={user}/></>;}
