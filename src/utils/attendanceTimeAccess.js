...
export function attendanceAccessReasonVi(result = {}) {
  switch (result.reason) {
    ...
    case 'outside_time': return `Tài khoản được phân quyền chỉ được thao tác điểm danh trong khung giờ ${result.windowLabel || 'Admin đã quy định'}.`;
    case 'missing_permission': return 'Tài khoản chưa được Admin cấp quyền Điểm danh nhanh.';
    case 'invalid_now': return 'Không thể xác định thời gian hiện tại theo múi giờ Việt Nam.';
    default: return result.allowed ? 'Được phép điểm danh tất cả lớp.' : 'Không thể thao tác điểm danh lúc này.';
  }
}
