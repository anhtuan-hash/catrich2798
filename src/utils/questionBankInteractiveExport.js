...
function normalizeMotion(value) {
  return ['none', 'soft', 'lively'].includes(value) ? value : 'soft';
}

function normalizeDuration(value, fallback = 50) {
  const duration = Number(value);
  return Number.isFinite(duration) && duration >= 0 ? duration : fallback;
}

export function buildInteractiveExamPayload({ test, items = [], options = {} } = {}) {
  ...
  const questions = ordered.map((item, index) => {
    ...
    const visible = visibleOptions(item);
    ...
    return {
      ...
      o: visible.map((option) => valueText(option.text)),
      optionIds: visible.map((option, optionIndex) => valueText(option.id || option.key || String.fromCharCode(65 + optionIndex))),
      a: answerIndex(item),
      ctx: context,
  ...
  const testDuration = normalizeDuration(test?.settings?.durationMinutes, 50);
  return {
    schema: 'brian-interactive-assessment/v1',
    playerVersion: '3.2-safe',
    ...
    defaults: {
      durationMinutes: normalizeDuration(options.durationMinutes, testDuration),
...
export async function buildInteractiveExamHtml({
  test,
  items,
  options,
  templateUrl = '/templates/brian-interactive-exam-v3.2-safe.html',
} = {}) {
  const payload = buildInteractiveExamPayload({ test, items, options });
