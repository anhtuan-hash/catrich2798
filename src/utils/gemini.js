...
import { setServerAiStatus } from './aiProviders.js';

export const DEFAULT_GEMINI_MODEL = 'server-managed';
...
function normalizeMaxOutputTokens(value, fallback = DEFAULT_MAX_OUTPUT_TOKENS) {
  ...
  return Math.min(8192, Math.max(128, parsed));
}
...
async function accessToken() {
  if (!supabase) throw new Error('Supabase chưa được cấu hình; không thể xác thực AI Gateway.');
  const { data, error } = await supabase.auth.getSession();
  if (error || !data?.session?.access_token) {
    const authError = new Error('Phiên đăng nhập đã hết hạn. Hãy đăng nhập lại để sử dụng AI.');
    authError.code = 'AI_AUTH_REQUIRED';
    throw authError;
  }
...
function normalizeAttachments(items = []) {
  ...
  for (const item of Array.isArray(items) ? items.slice(0, 4) : []) {
    ...
    const dataUrl = String(item?.dataUrl || (item?.base64 ? `data:${mimeType};base64,${item.base64}` : ''));
    if (!dataUrl.startsWith('data:image/')) continue;
    if (total + dataUrl.length > 3_200_000) break;
...
export async function callAI(options = {}) {
  ...
  try {
    ...
    if (!response.ok || !data?.ok) {
      const error = new Error(data?.error || `AI Gateway trả về mã ${response.status}.`);
