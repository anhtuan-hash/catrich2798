          ...
          : workspaceView === 'history' && manager ? (
            <main className="ttcm-history-view" role="tabpanel" aria-label="Lịch sử và file giáo viên">
              ...
              <section className="ttcm-history-stats" aria-label="Tổng quan hoạt động">
                ...
                <article className="ttcm-history-stat is-files"><div className="ttcm-history-stat-head"><span className="ttcm-history-stat-icon"><Icon name="folder" size={16} /></span><span>Tệp đã nộp</span></div><strong>{historySummary.fileCount}</strong><small>Giữ đủ các lần nộp, không ghi đè lịch sử</small></article>
              ...
              <section className="ttcm-history-grid">
                <article className="ttcm-history-panel">
                  ...
                  {historyTeacherId && visibleHistoryTimeline.length ? <div className="ttcm-history-timeline ttcm-history-scroll-region">{visibleHistoryTimeline.map((entry) => { const expanded = expandedHistoryIds.has(entry.id); const expandable = historyBodyNeedsExpansion(entry.body); return <div className={`ttcm-history-event is-${entry.statusId || 'responded'}`} key={entry.id}><span className="ttcm-history-event-dot" /><div className="ttcm-history-event-card"><header><b>{entry.label}</b><time>{formatDate(entry.createdAt)}</time></header><h4><button type="button" className="ttcm-history-item-link" onClick={() => openHistoryItem(entry.itemId)}>{entry.itemTitle}</button></h4>{entry.body ? <><p className={!expanded && expandable ? 'is-clamped' : undefined}>{entry.body}</p>{expandable ? <button type="button" className="ttcm-history-expand" onClick={() => toggleHistoryExpanded(entry.id)}>{expanded ? 'Thu gọn' : 'Xem đầy đủ'}</button> : null}</> : null}<div className="ttcm-history-event-primary"><span className={`ttcm-history-chip is-${historyItemTypeClass(entry.itemType)}`}>{entry.itemType}</span><span className={`ttcm-history-status is-${entry.statusId || 'responded'}`}>{entry.statusLabel || 'Đã phản hồi'}</span></div><div className="ttcm-history-meta-line"><span>Phản hồi #{entry.responseIndex}</span>{entry.attachmentCount ? <><span>· Lần nộp #{entry.fileSubmissionIndex}</span><span>· {entry.attachmentCount} tệp</span></> : null}</div></div></div>; })}</div> : <div className="ttcm-history-empty"><Icon name="history" size={30} /><strong>{historyTeacherId ? 'Chưa có hoạt động phù hợp' : 'Chọn giáo viên để xem lịch sử'}</strong><span>{historyTeacherId ? 'Các phản hồi, xác nhận và lần nộp tệp mới sẽ tự động xuất hiện tại đây.' : 'TTCM có thể chọn từng giáo viên trong tổ để xem hồ sơ hoạt động.'}</span></div>}
                </article>
                ...
                <article className="ttcm-history-panel">
                  ...
                  <div className="ttcm-history-filter-chips" aria-label="Bộ lọc file nhanh">{HISTORY_FILE_FILTERS.map((filterOption) => <button type="button" key={filterOption.id} className={`ttcm-history-filter-chip is-${filterOption.id} ${historyFileFilter === filterOption.id ? 'is-selected' : ''}`} aria-pressed={historyFileFilter === filterOption.id} onClick={() => setHistoryFileFilter(filterOption.id)}>{filterOption.label} {historyFileFilterCounts[filterOption.id] ?? 0}</button>)}</div>
                  {historyTeacherId && visibleHistoryFiles.length ? <div className="ttcm-history-files ttcm-history-scroll-region"><table className="ttcm-history-table"><thead><tr><th>Tệp</th><th>Nội dung TTCM</th><th>Ngày nộp</th><th>Lần nộp</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody>{visibleHistoryFiles.map((file) => { const ext = getWorkHubAttachmentExtension(file); const relatedItem = items.find((item) => String(item.id) === String(file.itemId)) || { id: file.itemId, title: file.itemTitle }; return <tr key={file.id}><td><div className="ttcm-history-file-name"><span className={`ttcm-history-file-badge is-${historyFileClass(ext)}`}>{ext ? ext.slice(0, 4).toUpperCase() : 'FILE'}</span><div><b title={file.name}>{file.name || 'Tệp đính kèm'}</b><small>{[ext ? ext.toUpperCase() : '', formatFileSize(file.size)].filter(Boolean).join(' · ') || 'Tệp TTCM'}</small></div></div></td><td><button type="button" className="ttcm-history-item-link" onClick={() => openHistoryItem(file.itemId)}>{file.itemTitle}</button><br /><small>{file.itemType}</small></td><td>{formatFullDate(file.submittedAt) || '—'}</td><td><span className={`ttcm-history-round ${file.submissionIndex > 1 ? 'is-later' : ''}`}>Lần {file.submissionIndex}</span></td><td><span className={`ttcm-history-status is-${file.statusId || 'submitted'}`}>{file.statusLabel || 'Đã nộp'}</span></td><td><div className="ttcm-history-actions"><button type="button" onClick={() => previewAttachment(relatedItem, file)} title="Xem trước" aria-label={`Xem ${file.name || 'tệp'}`}><Icon name="eye" size={17} /></button><button type="button" onClick={() => downloadAttachment(relatedItem, file)} title="Tải về" aria-label={`Tải ${file.name || 'tệp'}`}><Icon name="download" size={17} /></button></div></td></tr>; })}</tbody></table></div> : <div className="ttcm-history-empty"><Icon name="folder" size={30} /><strong>{historyTeacherId ? ((historyQuery || historyFileFilter !== 'all') ? 'Không tìm thấy tệp phù hợp' : 'Giáo viên chưa nộp tệp') : 'Chưa chọn giáo viên'}</strong><span>Toàn bộ tệp đính kèm trong các lần phản hồi TTCM sẽ được tập hợp ở đây và vẫn giữ riêng từng lần nộp.</span></div>}
                </article>
              </section>
            </main>
          )
