...
export function LearningTab({ workspace, onCommit, currentUser }) {
  ...
  const students = (workspace.students || []).filter((item) => item.active !== false);
  const selectedSubject = subjectFilter === 'all' ? '' : subjectFilter;
  const analytics = students.map((student) => ({ student, ...studentMetrics(workspace, student.id, { subject: selectedSubject }) }));
  const analyticsAverages = analytics.map((item) => item.average).filter(Number.isFinite);
  const selectedClassAverage = analyticsAverages.length ? analyticsAverages.reduce((sum, value) => sum + value, 0) / analyticsAverages.length : null;
  ...
  const save = async () => {
    ...
    try {
      const next = addLearningRecord(workspace, { ...draft, score: String(draft.score).replace(',', '.'), maxScore: String(draft.maxScore).replace(',', '.') });
      await onCommit(next, draft.id ? 'Đã cập nhật kết quả học tập.' : 'Đã thêm kết quả học tập.');
      setDraft((current) => ({ ...current, id: '', score: '', note: '', recordedAt: todayIso() }));
      setFormError('');
    } catch (error) { setFormError(error?.message || 'Không thể lưu điểm.'); }
  };
  ...
  const importAll = async () => {
    let next = workspace; let added = 0; let skipped = 0; const errors = [];
    importState.rows.forEach((row) => {
      ...
      if (!student) { skipped += 1; errors.push(`Dòng ${row.__row}: không khớp học sinh`); return; }
      try {
        next = addLearningRecord(next, { ...row, studentId: student.id, subject: row.subject || draft.subject, period: row.period || draft.period, teacherName: row.teacherName || draft.teacherName });
        added += 1;
      } catch (error) { skipped += 1; errors.push(`Dòng ${row.__row}: ${error?.message || 'dữ liệu không hợp lệ'}`); }
    });
    if (added) await onCommit(next, `Đã nhập ${added} kết quả; bỏ qua ${skipped} dòng không hợp lệ.`);
    setFormError(skipped ? `${skipped} dòng bị bỏ qua. ${errors.slice(0, 3).join(' · ')}` : '');
