  ...
  return <section className="att-report-m3">
    ...
    {loading ? <div className="att-report-m3__loading">Đang tổng hợp dữ liệu {mode === 'day' ? 'ngày' : 'tháng'}…</div> : <>
      <div className="att-report-m3__metrics">
        ...
        <article className="is-green"><span>Lượt có mặt</span><b>{report.metrics.presentInstances}</b><small>{pct(report.metrics.attendanceRate)} chuyên cần · {report.metrics.lateInstances} đi trễ</small></article>
      ...
      <section className="att-report-m3__surface">
        ...
        <div className="att-report-m3__teacher-grid">{report.teacherRows.map((row) => <article key={row.teacher_name}><strong>{row.teacher_name}</strong><b>{periodLabel(row.total_periods)}</b><span>{row.completed_sessions} buổi · {row.distinct_classes} lớp</span><small>{row.present_instances} có mặt · {row.late_instances} đi trễ · {row.absent_instances} vắng · {pct(row.attendance_rate)}</small></article>)}{!report.teacherRows.length ? <div className="att-report-m3__empty">Chưa có buổi dạy phù hợp bộ lọc.</div> : null}</div>
      </section>
