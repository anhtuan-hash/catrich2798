  ...
  return (
    <section className="attendance-archive" aria-label="Kho lưu trữ điểm danh">
      ...
      <div className="attendance-archive__toolbar" data-bes-keep-search="true">
        ...
        <label><span>Trạng thái xóa</span><select value={requestStatus} onChange={(event) => setRequestStatus(event.target.value)}><option value="all">Tất cả</option><option value="none">Chưa yêu cầu</option><option value="pending">Chờ duyệt</option><option value="rejected">Đã từ chối</option></select></label>
      ...
      {!loading && filtered.length ? <div className="attendance-archive__list">{filtered.map((item) => {
        ...
        return (
          <article key={item.archive_id} className={`attendance-archive__card ${pending ? 'is-pending' : ''} ${rejected ? 'is-rejected' : ''}`}>
            ...
            <div className="attendance-archive__actions">
              <button type="button" className="is-restore" disabled={busy} onClick={() => onRestore?.(item)}>Khôi phục</button>
              {!pending ? <button type="button" className="is-request" disabled={busy} onClick={() => onRequestDelete?.(item)}>{rejected ? 'Gửi lại yêu cầu xóa' : 'Yêu cầu xóa vĩnh viễn'}</button> : null}
              {isAdmin && pending ? <>
