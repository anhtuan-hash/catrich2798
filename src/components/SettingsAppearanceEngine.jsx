...
const DEFAULTS = { accent:'violet',accentCustom:'#7447E8',accentMode:'global',density:'comfortable',motion:'balanced',background:'mesh',adaptivePerformance:true,reduceMotion:false,highContrast:false,projector:false,batterySaver:false };
...
function stored(){
 try{
  const next={...DEFAULTS,...JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}')};
  delete next.theme;
  delete next.textScale;
  next.projector=false;
  return next;
...
export default function SettingsAppearanceEngine({language='vi',setAccent,setDensity,setMotionMode,setPerformanceMode}){
 ...
 const syncApp=(next)=>{setAccent?.(appAccent(next.accent));setDensity?.(appDensity(next.density));setMotionMode?.(appMotion(next.motion));setPerformanceMode?.(next.adaptivePerformance?'auto':(next.batterySaver?'low':'high'))};
 useEffect(()=>{const sync=(event)=>{const next={...DEFAULTS,...(event?.detail?.state||window.BESAppearance?.getState?.()||stored())};delete next.theme;delete next.textScale;next.projector=false;setReady(Boolean(window.BESAppearance));setState(next);syncApp(next)};sync();window.addEventListener('bes:appearance-ready',sync);window.addEventListener('bes:appearance-changed',sync);return()=>{window.removeEventListener('bes:appearance-ready',sync);window.removeEventListener('bes:appearance-changed',sync)}},[]);
 const update=(patch)=>{const clean={...patch};delete clean.theme;delete clean.textScale;delete clean.projector;const next={...state,...clean,projector:false,updatedAt:Date.now()};setState(next);syncApp(next);if(window.BESAppearance?.setState)window.BESAppearance.setState({...clean,textScale:100,projector:false});else{try{localStorage.setItem(STORAGE_KEY,JSON.stringify({...next,textScale:100,projector:false}))}catch{}window.dispatchEvent(new CustomEvent('bes:appearance-changed',{detail:{state:{...next,textScale:100,projector:false}}}))}};
