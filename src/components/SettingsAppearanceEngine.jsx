...
const NATIVE_LAYOUT = { density:'comfortable',contentWidth:'wide',textScale:100,projector:false,touchTargets:'normal' };
const DEFAULTS = { accent:'violet',accentCustom:'#7447E8',accentMode:'global',motion:'balanced',background:'mesh',adaptivePerformance:true,reduceMotion:false,highContrast:false,batterySaver:false,...NATIVE_LAYOUT };
...
function stored(){
 try{
  const next={...DEFAULTS,...JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}'),...NATIVE_LAYOUT};
  delete next.theme;
  return next;
...
export default function SettingsAppearanceEngine({language='vi',setAccent,setMotionMode,setPerformanceMode}){
 ...
 useEffect(()=>{const sync=(event)=>{const next={...DEFAULTS,...(event?.detail?.state||window.BESAppearance?.getState?.()||stored()),...NATIVE_LAYOUT};delete next.theme;setReady(Boolean(window.BESAppearance));setState(next);syncApp(next)};sync();window.addEventListener('bes:appearance-ready',sync);window.addEventListener('bes:appearance-changed',sync);return()=>{window.removeEventListener('bes:appearance-ready',sync);window.removeEventListener('bes:appearance-changed',sync)}},[]);
 const update=(patch)=>{const clean={...patch};delete clean.theme;delete clean.density;delete clean.contentWidth;delete clean.textScale;delete clean.projector;delete clean.touchTargets;const next={...state,...clean,...NATIVE_LAYOUT,updatedAt:Date.now()};setState(next);syncApp(next);if(window.BESAppearance?.setState)window.BESAppearance.setState({...clean,...NATIVE_LAYOUT});else{try{localStorage.setItem(STORAGE_KEY,JSON.stringify(next))}catch{}window.dispatchEvent(new CustomEvent('bes:appearance-changed',{detail:{state:next}}))}};
 ...
 return <div className="settings-engine-integrated">
  <div className="settings-engine-banner"><div className="settings-engine-mark" aria-hidden="true"><i/><i/><i/><i/></div><div><strong>Appearance Engine V2</strong><small>{vi?'Màu sắc, chuyển động và Adaptive UI. Kích thước chữ và bố cục dùng giá trị gốc của hệ thống.':'Color, motion and Adaptive UI. Text and layout sizing use native system values.'}</small></div><span className={ready?'is-ready':''}><i/>{ready?(vi?'Đang hoạt động':'Active'):(vi?'Đang kết nối':'Connecting')}</span><button type="button" onClick={()=>window.BESAppearance?.open?.('color')}>{vi?'Mở nâng cao':'Advanced'} ↗</button></div>
  ...
  <section className="settings-engine-adaptive"><header><div><strong>Adaptive UI</strong><small>{vi?'Chỉ tối ưu hiệu năng, chuyển động và tương phản; không thay đổi cỡ chữ hay kích thước nội dung.':'Optimizes performance, motion and contrast only; it no longer changes text or content size.'}</small></div><b>{String(tier).toUpperCase()}</b></header><div>{[["adaptivePerformance",vi?'Tự tối ưu hiệu suất':'Adaptive performance'],["reduceMotion",vi?'Giảm chuyển động':'Reduce motion'],["highContrast",vi?'Tương phản cao':'High contrast'],["batterySaver",vi?'Tiết kiệm pin':'Battery saver']].map(([key,label])=><label key={key}><span>{label}</span><Toggle checked={Boolean(state[key])} onChange={(value)=>update({[key]:value})} label={label}/></label>)}</div></section>
 </div>
}
