...
export function ClassLifecycleTab({ workspace, catalog, currentId, onSwitch, onCreate, onDuplicate, onStatusChange, currentUser }) {
  const [draft, setDraft] = useState({ className: '', schoolYear: workspace.classProfile.schoolYear || '', semester: 'Học kỳ I', grade: workspace.classProfile.grade || '12', room: '', classType: SUBJECT_CLASS_TYPE });
  const [copy, setCopy] = useState({ className: `${workspace.classProfile.className || 'Lớp'} · Năm học mới`, schoolYear: workspace.classProfile.schoolYear || '', semester: 'Học kỳ I', classType: SUBJECT_CLASS_TYPE, keepHistory: false });
  const active = catalog.filter((item) => item.status !== 'archived');
  const archived = catalog.filter((item) => item.status === 'archived');
  const homeroom = active.find((item) => item.classType === 'homeroom');
  const create = async () => {
  ...
  return <div className="hr-tab-stack">
    <section className="hr-panel hr-phase3-banner"><div><small>PHÂN LOẠI LỚP GIẢNG DẠY</small><h2>Một lớp chủ nhiệm, nhiều lớp bộ môn</h2><p>Mỗi lớp có không gian riêng. Lớp bộ môn chỉ giữ danh sách học sinh và sổ điểm; dữ liệu GVCN được ẩn nhưng không bị xóa.</p></div><span>{homeroom ? `GVCN · ${homeroom.className}` : 'Chưa chọn lớp chủ nhiệm'}<small>{active.length} lớp đang hoạt động</small></span></section>

    <section className="hr-panel">
      <div className="hr-panel-head"><div><small>Không gian hiện có</small><h2>Chuyển nhanh giữa các lớp</h2></div></div>
      <div className="hr-class-catalog">{active.map((item) => <article key={item.id} className={`${item.id === currentId ? 'current ' : ''}is-${item.classType || SUBJECT_CLASS_TYPE}`}><header><span>{item.grade || 'Lớp'}</span><div><div className={`hr-class-type-badge ${item.classType || SUBJECT_CLASS_TYPE}`}>{getClassTypeLabel(item.classType)}</div><h3>{item.className}</h3><small>{item.schoolYear || '—'} · {item.semester || '—'}</small></div></header><div className="hr-class-card-stats"><b>{item.studentCount || 0}<small>học sinh</small></b><b>{formatDateTime(item.updatedAt)}<small>cập nhật</small></b></div><footer>{item.id === currentId ? <strong>Đang mở</strong> : <button type="button" onClick={() => onSwitch(item.id)}>Mở lớp</button>}<button type="button" className="danger" disabled={item.id === currentId && active.length <= 1} onClick={() => onStatusChange(item.id, 'archived')}>Lưu trữ</button></footer></article>)}</div>
      {!active.length ? <p className="hr-muted">Chưa có lớp đang hoạt động.</p> : null}
    </section>

    <section className="hr-two-column">
      <article className="hr-panel"><div className="hr-panel-head"><div><small>Tạo mới</small><h2>Thêm lớp giảng dạy</h2></div><button type="button" className="primary" onClick={create}>Tạo lớp</button></div><div className="hr-form-grid two"><label><span>Loại lớp</span><select value={draft.classType} onChange={(e) => setDraft({ ...draft, classType: e.target.value })}>{HOMEROOM_CLASS_TYPE_OPTIONS.map((item) => <option key={item.value} value={item.value}>{item.labelVi}</option>)}</select></label><label><span>Tên lớp</span><input value={draft.className} onChange={(e) => setDraft({ ...draft, className: e.target.value })} placeholder="12.6" /></label><label><span>Năm học</span><input value={draft.schoolYear} onChange={(e) => setDraft({ ...draft, schoolYear: e.target.value })} /></label><label><span>Học kỳ</span><select value={draft.semester} onChange={(e) => setDraft({ ...draft, semester: e.target.value })}><option>Học kỳ I</option><option>Học kỳ II</option><option>Cả năm</option><option>Hè</option></select></label><label><span>Khối</span><input value={draft.grade} onChange={(e) => setDraft({ ...draft, grade: e.target.value })} /></label><label><span>Phòng</span><input value={draft.room} onChange={(e) => setDraft({ ...draft, room: e.target.value })} /></label><label><span>Giáo viên</span><input value={currentUser?.name || currentUser?.email || ''} readOnly /></label></div><p className="hr-security-note">Nếu chọn lớp chủ nhiệm khi đã có một lớp chủ nhiệm khác, hệ thống sẽ hỏi trước khi chuyển lớp cũ thành lớp bộ môn.</p></article>
      <article className="hr-panel"><div className="hr-panel-head"><div><small>Chuyển năm học</small><h2>Sao chép lớp hiện tại</h2></div><button type="button" className="primary" onClick={duplicate}>Tạo bản sao</button></div><div className="hr-form-grid two"><label><span>Loại lớp mới</span><select value={copy.classType} onChange={(e) => setCopy({ ...copy, classType: e.target.value })}>{HOMEROOM_CLASS_TYPE_OPTIONS.map((item) => <option key={item.value} value={item.value}>{item.labelVi}</option>)}</select></label><label><span>Tên lớp mới</span><input value={copy.className} onChange={(e) => setCopy({ ...copy, className: e.target.value })} /></label><label><span>Năm học mới</span><input value={copy.schoolYear} onChange={(e) => setCopy({ ...copy, schoolYear: e.target.value })} /></label><label><span>Học kỳ</span><select value={copy.semester} onChange={(e) => setCopy({ ...copy, semester: e.target.value })}><option>Học kỳ I</option><option>Học kỳ II</option><option>Cả năm</option></select></label><label className="hr-check-label"><input type="checkbox" checked={copy.keepHistory} onChange={(e) => setCopy({ ...copy, keepHistory: e.target.checked })} /><span>Giữ điểm, chuyên cần và lịch sử cũ</span></label></div><p className="hr-security-note">Mặc định hệ thống chỉ sao chép danh sách học sinh và cấu hình lớp; dữ liệu lịch sử không được mang sang năm học mới.</p></article>
    </section>

    {archived.length ? <section className="hr-panel"><div className="hr-panel-head"><div><small>{archived.length} lớp</small><h2>Kho lớp đã lưu trữ</h2></div></div><div className="hr-archive-list">{archived.map((item) => <article key={item.id}><div><span className={`hr-class-type-badge ${item.classType || SUBJECT_CLASS_TYPE}`}>{getClassTypeLabel(item.classType)}</span><b>{item.className}</b><small>{item.schoolYear} · lưu trữ {formatDateTime(item.archivedAt)}</small></div><button type="button" onClick={() => onStatusChange(item.id, 'active')}>Khôi phục</button><button type="button" onClick={() => onSwitch(item.id)}>Xem dữ liệu</button></article>)}</div></section> : null}
  </div>;
}
