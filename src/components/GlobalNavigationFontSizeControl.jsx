...
import './GlobalNavigationFontSizeControl.css';
...
export default function GlobalNavigationFontSizeControl({ currentUser, language = 'vi' }) {
  ...
  const commit = async (requestedSize) => {
    ...
    const result = await saveRegionalFontSettings(next, currentUser);
    if (!result?.ok) {
  ...
  return createPortal(
    <div
      ...
      data-navigation-font-size-control="true"
