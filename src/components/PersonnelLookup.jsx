...
const COPY = {
  vi: {
    ...
    search: 'Tìm theo tên, email, chuyên ngành hoặc phân công', total: 'Tổng nhân sự', postgraduate: 'Sau đại học', masters: 'Thạc sĩ', incomplete: 'Thiếu hồ sơ', pending: 'Chờ duyệt',
    all: 'Tất cả', doctorate: 'Tiến sĩ', doctoral_candidate: 'Nghiên cứu sinh', master: 'Thạc sĩ', bachelor: 'Cử nhân', missing: 'Chưa cập nhật', pendingFilter: 'Đề xuất chờ duyệt',
  ...
  en: {
    ...
    search: 'Search name, email, major or assignment', total: 'Total staff', postgraduate: 'Postgraduate', masters: "Master's", incomplete: 'Incomplete', pending: 'Pending review',
    all: 'All', doctorate: 'Doctorate', doctoral_candidate: 'Doctoral candidate', master: "Master's", bachelor: "Bachelor's", missing: 'Not updated', pendingFilter: 'Pending proposals',
...
const MATERIAL_PATHS = {
  groups: 'M16 11c1.66 0 3-1.34 3-3s-1.34-3-3-3 1.34-3 3 1.34 3 3 3Zm-8 0c1.66 0 3-1.34 3-3S9.66 5 8 5 5 6.34 5 8s1.34 3 3 3Zm0 2c-2.33 0-7 1.17-7 3.5V19h14v-2.5C15 14.17 10.33 13 8 13Zm8 0c-.29 0-.62.02-.97.05 1.16.84 1.97 1.97 1.97 3.45V19h6v-2.5c0-2.33-4.67-3.5-7-3.5Z',
...
export default function PersonnelLookup({ currentUser, language = 'vi' }) {
  ...
  const people = useMemo(() => accounts.map((user) => {
    ...
    const fallback = emptyPersonnelRecord({
      position: isLeader(user) ? t.positionTtcm : t.positionTeacher,
  ...
  const filteredPeople = useMemo(() => {
    ...
    return people.filter((person) => {
      const incomplete = !person.degreeLevel || !person.major;
      const matchesFilter = filter === 'all'
        || (filter === 'missing' ? incomplete : false)
        || (filter === 'pending' ? person.workflowStatus === 'submitted' : false)
        || person.degreeLevel === filter;
      if (!matchesFilter) return false;
      if (!needle) return true;
      return [person.name, person.email, person.position, person.major, person.specialization, person.assignment, person.institution, degreeLabel(person.degreeLevel, language)]
  ...
  const startEditing = () => {
    if (!selected) return;
    const source = !leaderView && selected.proposedProfile
      ? selected.proposedProfile
      : selected.approvedProfile || selected;
    setDraft(cleanPersonnelRecord(source));
    setProposalNote(selected.proposalNote || '');
    setEditing(true);
    setNotice('');
  };
  ...
  const saveProfile = async (event) => {
    ...
    const result = leaderView
      ? await saveApprovedPersonnelProfile({ currentUser, targetUser: selected, record: draft, existingItem: selected.item })
      : await submitPersonnelProposal({ currentUser, record: draft, proposalNote, existingItem: selected.item });
    setBusy(false);
    if (!result.ok) {
  ...
  const reviewProposal = async (decision) => {
    ...
    setBusy(true);
    setLoadError('');
    const result = await reviewPersonnelProposal({
      currentUser,
      targetUser: selected,
      existingItem: selected.item,
      decision,
      reviewNote,
    });
    setBusy(false);
  ...
  const profileFields = useMemo(() => [
    ['degreeLevel', t.highestLevel, (value) => degreeLabel(value, language) || '—'],
    ['degreeName', t.degreeName], ['major', t.major], ['specialization', t.specialization],
    ['institution', t.institution], ['graduationYear', t.graduationYear], ['position', t.position],
    ['department', t.department], ['employmentType', t.employmentType, (value) => employmentLabel(value, t)],
    ['employmentStatus', t.status, (value) => statusLabel(value, t)], ['assignment', t.assignment],
    ['phone', t.phone], ['otherDegrees', t.otherDegrees],
  ], [language]);
  ...
  const changedFields = useMemo(() => {
    ...
    return profileFields.filter(([key]) => String(current[key] || '') !== String(selected.proposedProfile[key] || ''));
  ...
  const renderProfileDetails = (person) => <div className="gpl-profile-content">
    ...
    <section><h4><MaterialIcon name="school" size={19} />{t.professional}</h4>{person.approvedProfile ? <dl><div><dt>{t.highestLevel}</dt><dd className={person.degreeLevel ? '' : 'is-missing'}>{person.degreeLevel ? degreeLabel(person.degreeLevel, language) : t.noQualification}</dd></div><div><dt>{t.degreeName}</dt><dd>{person.degreeName || '—'}</dd></div><div><dt>{t.major}</dt><dd>{person.major || t.noMajor}</dd></div><div><dt>{t.specialization}</dt><dd>{person.specialization || '—'}</dd></div><div><dt>{t.institution}</dt><dd>{person.institution || '—'}</dd></div><div><dt>{t.graduationYear}</dt><dd>{person.graduationYear || '—'}</dd></div></dl> : <p className="gpl-empty-profile">{t.noApprovedProfile}</p>}{person.otherDegrees ? <div className="gpl-other-degrees"><span>{t.otherDegrees}</span>{person.otherDegrees.split('\n').filter(Boolean).map((item) => <p key={item}>{item}</p>)}</div> : null}</section>
  ...
  return <article className={`gpl-directory gpl-workflow${leaderView ? ' is-leader' : ' is-teacher'}`} id="dashboard-personnel" aria-labelledby="gpl-title">
    ...
    <div className="gpl-body">
      ...
      {leaderView ? <>
        ...
        <div className="gpl-table-wrap"><div className="gpl-table-head" aria-hidden="true"><span>{t.teacher}</span><span>{t.position}</span><span>{t.qualification}</span><span>{t.assignment}</span><span>{t.workflow}</span><span /></div><div className="gpl-table" aria-live="polite">
          ...
          {filteredPeople.map((person) => <button type="button" className={`gpl-person-row${person.workflowStatus === 'submitted' ? ' has-pending' : ''}`} key={person.id} onClick={() => openProfile(person)}><span className="gpl-person-cell"><span className="gpl-avatar">{person.avatarUrl ? <img src={person.avatarUrl} alt="" /> : initials(person.name)}</span><span className="gpl-person-name"><strong>{person.name}</strong><small>{person.email}</small></span></span><span className="gpl-position-cell"><strong>{person.position || t.positionTeacher}</strong><small>{employmentLabel(person.employmentType, t)}</small></span><span className={`gpl-qualification-cell${person.degreeLevel ? '' : ' is-missing'}`}><b>{person.degreeLevel ? degreeLabel(person.degreeLevel, language) : t.noQualification}</b><small>{person.major || t.noMajor}</small></span><span className="gpl-assignment-cell"><strong>{person.assignment || t.noAssignment}</strong><small>{person.department}</small></span><span><em className={`gpl-workflow-status is-${person.workflowStatus}`}>{statusLabel(person.workflowStatus, t)}</em></span><span className="gpl-row-arrow"><MaterialIcon name="arrow" size={20} /></span></button>)}
        </div></div>
        ...
        <div className="gpl-teacher-facts"><div><small>{t.highestLevel}</small><strong>{selected.degreeLevel ? degreeLabel(selected.degreeLevel, language) : t.noQualification}</strong><span>{selected.major || t.noMajor}</span></div><div><small>{t.assignment}</small><strong>{selected.assignment || t.noAssignment}</strong><span>{selected.department}</span></div><div><small>{t.lastUpdated}</small><strong>{formatMoment(selected.updatedAt, language)}</strong><span>{t.syncMessage}</span></div></div>
        <div className="gpl-teacher-actions"><button type="button" className="gpl-button outlined" onClick={() => openProfile(selected)}>{t.profile}<MaterialIcon name="arrow" size={18} /></button><button type="button" className="gpl-button filled" onClick={() => { openProfile(selected); setTimeout(startEditing, 0); }}><MaterialIcon name="edit" size={18} />{selected.proposedProfile ? t.updateProposal : t.propose}</button></div>
      </section> : null}
    </div>
    ...
    {selectedId && selected ? <div className="gpl-drawer-layer" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) { setSelectedId(''); setEditing(false); } }}><aside className="gpl-drawer gpl-workflow-drawer" role="dialog" aria-modal="true" aria-labelledby="gpl-profile-title">
      <header className="gpl-drawer-header"><button type="button" className="gpl-icon-button" onClick={() => { setSelectedId(''); setEditing(false); }} aria-label={t.close}><MaterialIcon name="close" size={22} /></button><span>{t.profile}</span>{!editing ? <button type="button" className="gpl-edit-button" onClick={startEditing}><MaterialIcon name="edit" size={18} />{leaderView ? t.edit : (selected.proposedProfile ? t.updateProposal : t.propose)}</button> : <span />}</header>
      ...
      {editing && draft ? <form className="gpl-form" onSubmit={saveProfile}><section><h4><MaterialIcon name="school" size={19} />{t.professional}</h4><div className="gpl-form-grid"><label><span>{t.highestLevel}</span><select value={draft.degreeLevel} onChange={(event) => setDraft({ ...draft, degreeLevel: event.target.value })}><option value="">{t.missing}</option>{DEGREE_LEVELS.map((level) => <option key={level.id} value={level.id}>{level[language === 'en' ? 'en' : 'vi']}</option>)}</select></label><label><span>{t.degreeName}</span><input value={draft.degreeName} onChange={(event) => setDraft({ ...draft, degreeName: event.target.value })} /></label><label className="is-wide"><span>{t.major}</span><input value={draft.major} onChange={(event) => setDraft({ ...draft, major: event.target.value })} placeholder={t.suggestedMajor} /></label><label className="is-wide"><span>{t.specialization}</span><input value={draft.specialization} onChange={(event) => setDraft({ ...draft, specialization: event.target.value })} placeholder={t.suggestedSpecialization} /></label><label className="is-wide"><span>{t.institution}</span><input value={draft.institution} onChange={(event) => setDraft({ ...draft, institution: event.target.value })} placeholder={t.suggestedInstitution} /></label><label><span>{t.graduationYear}</span><input inputMode="numeric" value={draft.graduationYear} onChange={(event) => setDraft({ ...draft, graduationYear: event.target.value })} maxLength={4} /></label><label className="is-wide"><span>{t.otherDegrees}</span><textarea value={draft.otherDegrees} onChange={(event) => setDraft({ ...draft, otherDegrees: event.target.value })} placeholder={t.suggestedOther} rows={3} /></label></div></section><section><h4><MaterialIcon name="work" size={19} />{t.employment}</h4><div className="gpl-form-grid"><label><span>{t.position}</span><input value={draft.position} onChange={(event) => setDraft({ ...draft, position: event.target.value })} /></label><label><span>{t.department}</span><input value={draft.department} onChange={(event) => setDraft({ ...draft, department: event.target.value })} /></label><label><span>{t.employmentType}</span><select value={draft.employmentType} onChange={(event) => setDraft({ ...draft, employmentType: event.target.value })}><option value="core">{t.core}</option><option value="visiting">{t.visiting}</option></select></label><label><span>{t.status}</span><select value={draft.employmentStatus} onChange={(event) => setDraft({ ...draft, employmentStatus: event.target.value })}><option value="active">{t.active}</option><option value="leave">{t.leave}</option><option value="inactive">{t.inactive}</option></select></label><label className="is-wide"><span>{t.assignment}</span><input value={draft.assignment} onChange={(event) => setDraft({ ...draft, assignment: event.target.value })} placeholder={t.suggestedAssignment} /></label><label className="is-wide"><span>{t.phone}</span><input value={draft.phone} onChange={(event) => setDraft({ ...draft, phone: event.target.value })} inputMode="tel" /></label></div></section>{!leaderView ? <section className="gpl-proposal-note"><h4><MaterialIcon name="send" size={19} />{t.proposalNote}</h4><textarea value={proposalNote} onChange={(event) => setProposalNote(event.target.value)} placeholder={t.proposalNotePlaceholder} rows={3} /></section> : null}<div className="gpl-form-actions"><button type="button" className="gpl-button outlined" onClick={() => { setEditing(false); setDraft(null); }}>{t.cancel}</button><button type="submit" className="gpl-button filled" disabled={busy}><MaterialIcon name={leaderView ? 'save' : 'send'} size={18} />{busy ? t.loading : (leaderView ? t.save : t.submitProposal)}</button></div></form> : <>
        {renderProfileDetails(selected)}
        {selected.proposedProfile ? <section className="gpl-review-card"><header><div><span>{t.proposalHeading}</span><h4>{t.reviewHeading}</h4></div><em className={`gpl-workflow-status is-${selected.workflowStatus}`}>{statusLabel(selected.workflowStatus, t)}</em></header>{selected.proposalNote ? <p className="gpl-proposal-message"><MaterialIcon name="send" size={18} />{selected.proposalNote}</p> : null}<div className="gpl-compare-list">{changedFields.length ? changedFields.map(([key, label, formatter]) => { const current = selected.approvedProfile?.[key] || ''; const proposed = selected.proposedProfile?.[key] || ''; const show = formatter || ((value) => value || '—'); return <div className="gpl-compare-row" key={key}><strong>{label}</strong><span><small>{t.currentValue}</small><b>{show(current)}</b></span><MaterialIcon name="arrow" size={18} /><span className="is-proposed"><small>{t.proposedValue}</small><b>{show(proposed)}</b></span></div>; }) : <p className="gpl-empty-profile">{t.noChanges}</p>}</div>{leaderView && selected.workflowStatus === 'submitted' ? <div className="gpl-review-actions"><label><span>{t.reviewNote}</span><textarea value={reviewNote} onChange={(event) => setReviewNote(event.target.value)} placeholder={t.reviewNotePlaceholder} rows={3} /></label><div><button type="button" className="gpl-button danger" onClick={() => reviewProposal('changes')} disabled={busy}><MaterialIcon name="reject" size={18} />{t.requestChanges}</button><button type="button" className="gpl-button filled" onClick={() => reviewProposal('approve')} disabled={busy}><MaterialIcon name="approve" size={18} />{t.approve}</button></div></div> : selected.reviewNote ? <div className="gpl-review-feedback"><strong>{t.reviewerFeedback}</strong><p>{selected.reviewNote}</p></div> : null}</section> : selected.reviewNote ? <section className="gpl-review-feedback"><strong>{t.reviewerFeedback}</strong><p>{selected.reviewNote}</p></section> : null}
      </>}
    </aside></div> : null}
  </article>;
}
