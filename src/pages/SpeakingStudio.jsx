...
export default function SpeakingStudio({ language, apiKey, aiModel, hasApiKey }) {
  ...
  const generateCards = async () => {
    ...
    try {
      ...
      const text = await callAI({ apiKey, model: aiModel, prompt, temperature: 0.65, responseMimeType: 'application/json' });
  ...
  const gradeSpeaking = async () => {
    ...
    try {
      ...
      const text = await callAI({ apiKey, model: aiModel, prompt, temperature: 0.35, responseMimeType: 'application/json' });
      const json = extractJson(text);
      setScore(json);
