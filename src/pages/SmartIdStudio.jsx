...
export default function SmartIdStudio({ language = 'vi', apiKey = '', aiProvider = 'openrouter' }) {
  ...
  const analyzeFaceQuality = async (imageData) => {
    if (!imageData) return;
    setIsAnalyzing(true);
    const operation = { id: `smartid-analysis-${Date.now()}`, provider: 'OpenRouter Server Gateway', model: 'server-managed', label: t.analyzing };
    emitAi('bes-ai-operation-start', operation);
    try {
      const mimeType = dataUrlParts(imageData).mimeType;
      const text = await callAI({
        ...
        attachments: [{ mimeType, dataUrl: imageData }],
        temperature: 0.25,
        maxOutputTokens: 260,
        profile: 'document',
        clientLabel: 'SmartID portrait readiness analysis',
      });
      setAiFeedback(text || t.aiFeedbackDefault);
    } catch {
  ...
  const handleAiEdit = async () => {
    ...
    setErrorMsg('');
    setStatusMsg('');
    const operation = { id: `smartid-edit-${Date.now()}`, provider: 'OpenRouter Server Gateway', model: 'server-managed', label: t.processing };
    emitAi('bes-ai-operation-start', operation);
    try {
      const mimeType = dataUrlParts(sourceImage).mimeType;
      const instruction = aiPrompt.trim() || (language === 'vi' ? 'Làm sạch ảnh thẻ tự nhiên và chuyên nghiệp.' : 'Create a clean, natural professional ID portrait.');
      const advice = await callAI({
        prompt: `Review this ID portrait and provide a concise editing plan. User instruction: ${instruction}. Explain crop, lighting, background and clothing improvements in ${language === 'vi' ? 'Vietnamese' : 'English'}. Do not identify the person.`,
        attachments: [{ mimeType, dataUrl: sourceImage }],
        temperature: 0.3,
        maxOutputTokens: 420,
        profile: 'document',
        clientLabel: 'SmartID edit guidance',
      });
      setAiFeedback(advice || t.aiFeedbackDefault);
      setProcessedImage(sourceImage);
      setStatusMsg(language === 'vi' ? 'OpenRouter đã phân tích ảnh. Các công cụ cắt, nền và in vẫn xử lý cục bộ để bảo vệ danh tính.' : 'OpenRouter analyzed the photo. Crop, background and print tools remain local to protect identity.');
    } catch (error) {
        ...
        <div className="smartid-editor-shell">
          ...
          <aside className="smartid-editor-panel">
            <header className="smartid-panel-heading">
              ...
              <span className={`smartid-connection ${serverAiReady ? 'ready' : ''}`}>{serverAiReady ? t.aiReady : t.aiMissing}</span>
            </header>

            <div className="smartid-controls-scroll">
              {!serverAiReady ? <button type="button" className="smartid-inline-settings" onClick={() => { window.location.hash = '#/settings'; }}><Icon name="settings" size={18}/>{t.settings}</button> : null}
            ...
            <footer className="smartid-editor-footer">
              <button type="button" disabled={isProcessing || !serverAiReady} onClick={handleAiEdit}><Icon name="sparkle" size={22}/>{t.generate}</button>
            </footer>
          </aside>
        </div>
      )}
