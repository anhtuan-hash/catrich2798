...
assert.match(
  shell,
  /bes-supplemental-open-rollcall/,
  'The native React attendance surface must listen for class-launched supplemental rollcall.',
);
assert.match(
  shell,
  /bes-open-supplemental-attendance/,
  'The native React attendance surface must also accept supplemental rollcall from schedule/calendar entry points.',
);
assert.match(
  shell,
  /supplementalRollcall|rollcallSource|attendanceSource/,
  'The native attendance surface must keep an explicit source discriminator for supplemental sessions.',
);
assert.match(
  shell,
  /beginSupplementalAttendance/,
  'The native attendance surface must load/freeze the supplemental roster through the supplemental API.',
);
assert.match(
  shell,
  /confirmSupplementalAttendance/,
  'The native attendance surface must finalize supplemental attendance through the supplemental backend.',
);
assert.match(
  shell,
  /attachSupplementalProof/,
  'The native attendance surface must attach proof to supplemental sessions through the supplemental backend.',
);
assert.match(
  shell,
  /\[\[1,'1 tiết'\],\[1\.5,'1,5 tiết'\],\[2,'2 tiết'\]\]/,
  'The shared native rollcall must retain the 1 / 1.5 / 2 lesson-period selector.',
);
assert.match(
  shell,
  /Giáo viên dạy hôm nay/,
  'The shared native rollcall must retain the teacher selector.',
);
assert.match(
  shell,
  /Phòng học/,
  'The shared native rollcall must retain room editing.',
);
assert.match(
  shell,
  /Thời gian dạy/,
  'The shared native rollcall must retain teaching-time editing.',
);

assert.doesNotMatch(
  quickBootstrap,
  /id = ROLLCALL_ID|bes-supplemental-participant|bes-supplemental-rollcall" role="dialog"/,
  'The legacy bespoke supplemental rollcall DOM must be retired once native rollcall owns the flow.',
);
...
assert.match(
  reportUtils,
  /lessonPeriods|lesson_periods/,
  'Unified reporting must consume the supplemental lesson-period value rather than always assuming one period.',
);
