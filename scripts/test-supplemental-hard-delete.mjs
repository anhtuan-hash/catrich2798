...
assert.match(ui, /xóa vĩnh viễn|xóa hoàn toàn/i, 'Permanent deletion must be stated clearly in confirmation copy.');
...
assert.match(migration, /private\.bes_require_supplemental_manager\(\)/i, 'Hard-delete RPC must retain strict supplemental authorization.');
assert.match(migration, /delete from public\.bes_supplemental_attendance_record_changes/i, 'Hard delete must clear correction audit rows.');
...
assert.match(migration, /grant execute on function public\.bes_delete_supplemental_class\(uuid\) to authenticated/i, 'Authenticated callers use the manager-guarded RPC.');
...
assert.match(migration, /g\.archived_at is null/i, 'Attendance schedule must exclude archived parent classes.');
assert.match(migration, /g\.active = true/i, 'Attendance schedule must exclude inactive parent classes.');
assert.match(migration, /s\.group_id is null/i, 'Legacy adhoc sessions without a parent group must remain readable.');
