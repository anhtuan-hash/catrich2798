...
const required=['src/config/version.js','src/pages/LessonPack.jsx','src/pages/ClassroomDelivery.jsx','src/pages/ClassroomJoin.jsx','src/utils/classroomDelivery.js','src/styles/v1110.css','supabase/brian_v11_1_preflight.sql','supabase/brian_v11_1_classroom_delivery.sql','supabase/brian_v11_1_verify.sql','public/version.json','public/release-manifest.json'];
let failed=0;
for(const file of required){const ok=fs.existsSync(file);console.log(`${ok?'✓':'✗'} ${file}`);if(!ok)failed++;}
const pkg=JSON.parse(fs.readFileSync('package.json','utf8'));
const version=JSON.parse(fs.readFileSync('public/version.json','utf8'));
const main=fs.readFileSync('src/main.jsx','utf8');
if(pkg.version!=='11.1.0'||version.version!=='11.1.0'||version.runtimeCore!=='2.1.0'){console.error('✗ version registry mismatch');failed++;}else console.log('✓ version registry synchronized');
if(!main.includes("currentRoute === 'classroom-delivery'")||!main.includes("currentRoute === 'classroom-join'")||!main.includes("./styles/v1110.css")){console.error('✗ classroom routes or styles missing');failed++;}else console.log('✓ classroom routes and styles loaded');
const forbidden=[];
...
console.log('V11.1 release guard passed.');
