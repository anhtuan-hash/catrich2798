...
assert.match(css, /\.attendance-rollcall\s+\.attendance-roster\.attendance-roster\s*\{[\s\S]*?overflow:\s*visible\s*!important;/, 'Student roster must not keep its own nested scroll');
assert.match(css, /\.attendance-rollcall\s+\.attendance-roster\.attendance-roster\s*\{[\s\S]*?max-height:\s*none\s*!important;/, 'Student roster must expand naturally instead of being height-capped');
