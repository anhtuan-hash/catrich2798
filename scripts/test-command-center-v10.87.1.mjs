...
const harnessPath = path.join(cwd, 'tests', 'command-center-v10.87.1.harness.html');
...
for (const marker of cssMarkers) {
  if (!css.includes(marker)) fail(`CSS thiếu marker ${marker}`);
}
console.log(`✓ Static checks: ${requiredMarkers.length + cssMarkers.length}/${requiredMarkers.length + cssMarkers.length}`);
...
const server = http.createServer((req, res) => {
  ...
  if (url.pathname === '/bes-command-center-v10871.js') { file = jsPath; type = 'text/javascript; charset=utf-8'; }
  if (url.pathname === '/bes-command-center-v10871.css') { file = cssPath; type = 'text/css; charset=utf-8'; }
...
const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'bescc-v10871-'));
const args = [
  '--headless',
  '--no-sandbox',
  '--disable-gpu',
...
if (browserInfrastructureError) {
  ...
  console.log('✓ Command Center V10.87.1 static checks completed');
  process.exit(0);
}
...
if (/data-result="failed"/.test(output.stdout)) fail('Browser smoke test chạy nhưng UI hoặc lớp cách ly style không đạt.');
if (!/data-result="passed"/.test(output.stdout)) {
  console.log('! Chromium không trả DOM kiểm thử; static checks vẫn đạt.');
  console.log('✓ Command Center V10.87.1 static checks completed');
  process.exit(0);
}
if (!/data-style-isolated="true"/.test(output.stdout)) fail('Style của ứng dụng nền vẫn ảnh hưởng đến Command Center.');
console.log('✓ Browser smoke test: passed');
console.log('✓ Host-style isolation: passed');
console.log('✓ Command Center V10.87.1 checks completed');
