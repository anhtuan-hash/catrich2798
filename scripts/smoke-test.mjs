...
add('V11.6.7 server AI token budget applied globally', geminiSource.includes('maxOutputTokens: normalizeMaxOutputTokens') && fs.readFileSync(new URL('../server/openrouterGateway.js', import.meta.url), 'utf8').includes('resolveOutputLimit'), 'browser requests and server profile caps are enforced');
add('V11.6.7 OpenRouter affordable-token retry present', fs.readFileSync(new URL('../server/openrouterGateway.js', import.meta.url), 'utf8').includes('can only afford') && fs.readFileSync(new URL('../server/openrouterGateway.js', import.meta.url), 'utf8').includes('affordable - 24'), 'credit-limited requests retry server-side with a smaller cap');
...
add('V11.6.7 multimodal payloads use the single gateway', geminiSource.includes('attachments: normalizeAttachments') && fs.readFileSync(new URL('../server/openrouterGateway.js', import.meta.url), 'utf8').includes("type: 'image_url'"), 'images are normalized in the browser and sent to OpenRouter only on the server');
...
add('V11.6.7 Apps discovery keeps search and density without the recent rail', webAppsSource.includes('launcher-search-box') && !webAppsSource.includes('launcher-recent-strip') && webAppsSource.includes('bes-launcher-density') && webAppsSource.includes('normalizedSearch'), 'recent apps UI removed while search and compact/comfortable views remain');
...
add('V11.6.7 Governance Center exposes shared model, quotas and server audit', aiGovernancePageSource.includes('dailyRequestLimit') && aiGovernancePageSource.includes('profiles') && aiGovernancePageSource.includes("fetch('/api/ai?scope=governance'") && aiGovernancePageSource.includes('audit'), 'Admin controls shared runtime settings and reads server logs');
