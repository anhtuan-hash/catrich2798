...
const required=[
  ...
  'scripts/check-v11.4.9.mjs','scripts/sync-version-v11.4.9.mjs','scripts/performance-budget-v11.4.9.mjs',
  'public/version.json','public/release-manifest.json','.bes-release/v11.4.9.json',
];
let failed=0;
...
if(pkg.version!=='11.4.9'||version.version!=='11.4.9'||release.version!=='11.4.9'||version.runtimeCore!=='2.4.9'){console.error('✗ version registry mismatch');failed+=1;}else console.log('✓ version registry synchronized');
...
for(const marker of ['GRAMMAR BUILDER · V2.2','GRAMMAR_DOMAIN_GROUPS','Yêu cầu cụ thể khác','aria-pressed','has-value']){if(!grammar.includes(marker)){console.error(`✗ missing source marker ${marker}`);failed+=1;}else console.log(`✓ source marker ${marker}`);}
for(const marker of ['V11.4.9 — vivid selection system','modern dimensional cards','app-wide readable typography','.gb-mode-grid button.active::after','.gb-constraint-grid label.active','.gb-item-card.selected']){if(!css.includes(marker)){console.error(`✗ missing CSS marker ${marker}`);failed+=1;}else console.log(`✓ CSS marker ${marker}`);}
...
console.log('V11.4.9 release guard passed.');
