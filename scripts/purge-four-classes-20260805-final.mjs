...
async function main() {
  ...
  const result = {
    ok: verification.ok,
    purgeId: 'four-class-purge-2026-08-05-final-v2',
    classes: TARGET_CLASSES,
  ...
  console.log('[four-class-purge-final]', JSON.stringify(result));
  if (!verification.ok) process.exitCode = 1;
}

main().catch((error) => {
  console.error('[four-class-purge-final]', error);
  process.exitCode = 1;
});
