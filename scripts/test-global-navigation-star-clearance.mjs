...
assert.match(
  clearanceCss,
  /\.brian-nav\.brian-nav \.brian-nav__actions\s*\{[\s\S]*?position:\s*relative\s*!important;[\s\S]*?z-index:\s*4\s*!important;[\s\S]*?isolation:\s*isolate\s*!important;/,
);
...
assert.match(clearanceCss, /@media \(max-width: 1600px\) and \(min-width: 1281px\)/);
assert.match(
  clearanceCss,
  /@media \(max-width: 1600px\) and \(min-width: 1281px\)[\s\S]*?\.brian-nav\.brian-nav \.brian-nav__primary\s*\{[\s\S]*?gap:\s*2px\s*!important;/,
);
assert.match(
  clearanceCss,
  /@media \(max-width: 1600px\) and \(min-width: 1281px\)[\s\S]*?\.brian-nav\.brian-nav \.brian-nav__primary > :is\(button,a,\[role='button'\]\)[\s\S]*?padding:\s*0 7px 0 27px\s*!important;[\s\S]*?font-size:\s*11\.5px\s*!important;/,
);
assert.match(
  clearanceCss,
  /@media \(max-width: 1600px\) and \(min-width: 1281px\)[\s\S]*?\.brian-nav\.brian-nav \.brian-nav__account strong[\s\S]*?max-width:\s*70px\s*!important;/,
);
