...
const dashboardPage = read('src/pages/WorkDashboard.jsx');
const dashboardCss = read('src/styles/dashboard-iphone-readable.css');
const attendanceComponent = read('src/components/GlobalAttendanceNavigationTab.jsx');
...
assert.match(dashboardPage, /dashboard-iphone-readable\.css/, 'WorkDashboard must import the final iPhone stylesheet');
assert.match(attendanceComponent, /AttendanceIphoneReadable\.css/, 'Attendance must import the final iPhone stylesheet');
assert.match(navComponent, /GlobalIphoneNavigationReadable\.css/, 'Global navigation must import the final iPhone stylesheet');
...
assert.match(dashboardCss, /\.gd-agenda-list-split\s+\.gd-event-copy\s+strong\s*\{[^}]*font-size\s*:\s*(?:15|15\.5|16)px/i, 'Dashboard agenda titles must be 15-16px');
assert.match(dashboardCss, /\.gd-agenda-list-split\s+\.gd-event-time\s+strong\s*\{[^}]*font-size\s*:\s*(?:14|14\.5|15)px/i, 'Dashboard agenda times must be >=14px');
assert.match(dashboardCss, /\.gd-agenda-list-split\s+\.gd-event-copy\s+p\s*\{[^}]*display\s*:\s*-webkit-box/i, 'Dashboard event notes should be visible with controlled line clamping');
...
assert.match(attendanceCss, /\.attendance-roster[^}]*overflow-x\s*:\s*auto/i, 'Attendance roster overflow must be isolated inside the roster');
assert.match(attendanceCss, /padding-bottom[^;]*env\(safe-area-inset-bottom\)/i, 'Attendance confirmation area must clear the iPhone home indicator');
