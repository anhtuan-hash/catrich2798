...
const VERSION = '10.99.0';
const RELEASE = 'Production Hardening & Core Cleanup';
const RUNTIME = '1.6.0';
...
fs.writeFileSync('public/version.json', `${JSON.stringify({
  ...
  requiresSql: true,
  requiredMigration: 'brian_v10_99_production_hardening.sql',
  legacyDomPatches: false,
  generatedAt,
}, null, 2)}\n`);

fs.writeFileSync('public/release-manifest.json', `${JSON.stringify({
  version: VERSION,
  release: RELEASE,
  runtime: RUNTIME,
  schema: VERSION,
