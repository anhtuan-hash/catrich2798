...
const extra = normalizeExtraClassActivity({id:'e1',class_type:'gifted',class_name:'BD Anh 12',subject:'Anh',teacher_name:'GV B',attendance_date:'2026-09-11',teaching_time_range:'16:45–18:00',teaching_room:'B1',total_students:9,session_status:'completed'});
assert.equal(extra.activityType,'enrichment','production class_type=gifted must map to the Bồi dưỡng/enrichment filter');
assert.equal(extra.source,'extra');
assert.equal(filterAttendanceActivities([supplemental,extra],'supplemental').length,1);
assert.equal(filterAttendanceActivities([supplemental,extra],'enrichment').length,1);
