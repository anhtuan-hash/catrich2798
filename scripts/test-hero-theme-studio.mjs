...
const checks = [
  ...
  ['Image upload validates accepted raster types and calls optimizer', settings, /uploadHeroThemeStudioImage[\s\S]*image\/jpeg[\s\S]*image\/png[\s\S]*image\/webp[\s\S]*optimizeRaster/],
  ['Raster optimizer uses createImageBitmap and WebP delivery', settings, /createImageBitmap[\s\S]*image\/webp/],
