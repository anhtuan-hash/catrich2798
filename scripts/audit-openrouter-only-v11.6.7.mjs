...
add('Admin-only governance shares the existing AI function', api.includes("scope === 'governance'") && api.includes("roles: ['admin']") && api.includes('writeServerAiSettings'));
add('Admin governance page uses the authenticated shared gateway', governancePage.includes("fetch('/api/ai?scope=governance'") && governancePage.includes("currentUser?.role !== 'admin'"));
add('Standalone governance function is removed for Vercel Hobby', !fs.existsSync('api/ai-governance.js'));
