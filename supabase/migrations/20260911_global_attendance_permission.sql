-- 2026-09-11: make attendance operation permission global across all classes.
-- Admin grants attendance:quick once per account. Teaching assignment is not an
-- authorization signal. Report permission may bypass the time window only for
-- an account that also has attendance operation permission.
...
begin
  ...
  -- This helper is deliberately the global Admin-granted attendance operation
  -- gate. It accepts attendance:quick (and the legacy route:attendance grant),
  -- never a teacher/class assignment.
  if not public.can_take_extra_class_attendance() then
    return jsonb_build_object('allowed', false, 'reason', 'missing_permission');
  end if;

  -- Keep class validation for class-scoped RPCs, without using the class to
  -- decide who is authorized to operate attendance.
  ...
  -- Report permission only changes the time rule. It is intentionally checked
  -- after the global attendance-operation permission above.
...
comment on table public.bes_attendance_access_settings is
  'Admin-controlled global attendance window for accounts granted attendance:quick. Teaching assignment does not grant attendance operation rights.';
