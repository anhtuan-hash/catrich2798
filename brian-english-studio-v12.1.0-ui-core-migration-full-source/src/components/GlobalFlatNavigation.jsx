...
export default function GlobalFlatNavigation({
  ...
  return (
    <nav data-ui="navigation-primary" className="global-flat-navigation" aria-label={language === 'vi' ? 'Điều hướng toàn hệ thống' : 'Global navigation'}>
      <button type="button" data-ui="brand-button" data-ui="navigation-brand" className="global-flat-brand" onClick={(event) => go('#/home', 'BE', routeColors.home, event.currentTarget)}>
        <img className="global-flat-brand-logo" src="/brian-english-brand-mark.png" alt="Brian English logo" />
        <strong>Brian English</strong>
      </button>

      <div data-ui="navigation-items" className="global-flat-links" data-custom-launcher="true">
        {entries.map((entry) => (
          <button
            key={entry.id}
            type="button"
      ...
      <div data-ui="navigation-utilities" className="global-flat-actions">
        <button
          type="button"
