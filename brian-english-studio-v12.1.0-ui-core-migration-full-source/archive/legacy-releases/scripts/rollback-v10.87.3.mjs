...
function fail(message) {
  console.error(`\n[V10.87.3 rollback] ${message}\n`);
  process.exit(1);
}

if (!fs.existsSync(statePath)) fail('Không tìm thấy trạng thái cài đặt .bes-release/v10.87.3.json.');
const state = JSON.parse(fs.readFileSync(statePath, 'utf8'));
const backupDir = path.resolve(cwd, state.backupDir || '');
if (!state.backupDir || !fs.existsSync(backupDir)) fail('Không tìm thấy thư mục backup của V10.87.3.');
...
for (const asset of [
  ...
  'public/bes-release-v10.87.3.json'
]) {
...
fs.rmSync(statePath);
console.log('\nĐã rollback V10.87.3 về V10.87.2.');
console.log(`Đã dùng backup: ${state.backupDir}`);
console.log('Lịch sử hội thoại, draft, provider và API key không bị xóa.');
