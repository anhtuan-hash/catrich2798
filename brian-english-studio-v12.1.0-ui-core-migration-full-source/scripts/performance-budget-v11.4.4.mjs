...
const results=[
 ...
 ['Total CSS',sum(css),1500*1024,`${css.length} files`],
...
for(const [name,value,limit,detail] of results){const ok=value<=limit;if(!ok)failed++;console.log(`${ok?'PASS':'FAIL'}  ${name}: ${fmt(value)} / ${fmt(limit)} · ${detail}`);}
if(failed)process.exit(1);console.log('V11.4.4 performance budget passed.');
