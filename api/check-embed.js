...
function framePolicy(headers) {
  ...
  if (frameAncestors) {
    ...
    if (!/https:\/\/\*|\*|esl-pek\.vercel\.app|vercel\.app/i.test(frameAncestors)) return { embeddable: false, reason: `CSP frame-ancestors ${frameAncestors}` };
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    let response;
    try {
      response = await fetch(url, { method: 'HEAD', redirect: 'follow', signal: controller.signal, headers: { 'user-agent': 'Mozilla/5.0 (compatible; BrianEmbedCheck/1.0)', accept: 'text/html,*/*;q=0.5' } });
      if (response.status === 405 || response.status === 403) response = await fetch(url, { method: 'GET', redirect: 'follow', signal: controller.signal, headers: { 'user-agent': 'Mozilla/5.0 (compatible; BrianEmbedCheck/1.0)', accept: 'text/html,*/*;q=0.5', range: 'bytes=0-2048' } });
    } finally { clearTimeout(timeout); }
    return res.status(200).json({ ...framePolicy(response.headers), status: response.status, finalUrl: response.url || url.toString() });
  } catch (error) {
