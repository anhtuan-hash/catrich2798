...
const ENTITY_MAP = {
  ...
  rsquo: '’', lsquo: '‘', ldquo: '“', rdquo: '”', copy: '©', reg: '®', bull: '•', middot: '·',
};
...
function removeElementBlocks(html = '', tag = '') {
...
function candidateBlocks(html = '') {
  const starts = [];
  const tagRegex = /<(article|main|section|div)\b[^>]*>/gi;
  let match;
  ...
  return starts.map((entry) => {
    ...
    const pCount = (fragment.match(/<p\b/gi) || []).length;
    const headingCount = (fragment.match(/<h[2-4]\b/gi) || []).length;
...
function pushTextBlock(blocks, type, rawText) {
...
function blocksFromHtml(fragment = '', baseUrl = '') {
  ...
  while ((match = blockRegex.exec(cleaned))) {
    const whole = match[0];
    const type = (match[1] || 'img').toLowerCase();
    if (type === 'img') {
      const src = absoluteUrl(getAttr(whole, 'src') || getAttr(whole, 'data-src') || getAttr(whole, 'data-original') || getAttr(whole, 'data-lazy-src'), baseUrl);
      if (src && !blocks.some((item) => item.type === 'image' && item.src === src)) blocks.push({ type: 'image', src, alt: getAttr(whole, 'alt') || '' });
      continue;
    }
    if (type === 'figure') {
      const imgTag = whole.match(/<img\b[^>]*>/i)?.[0] || '';
      const src = absoluteUrl(getAttr(imgTag, 'src') || getAttr(imgTag, 'data-src') || getAttr(imgTag, 'data-original') || getAttr(imgTag, 'data-lazy-src'), baseUrl);
      const caption = normalizeText(whole.match(/<figcaption\b[^>]*>([\s\S]*?)<\/figcaption>/i)?.[1] || '');
...
function extractFromHtml(html, url) {
  ...
  if (jsonLd) {
    const blocks = blocksFromPlainText(jsonLd.articleBody);
    const text = textFromBlocks(blocks);
    if (wordCount(text) >= 60) {
      return {
        title: normalizeText(jsonLd.headline || jsonLd.name || findMeta(html, 'og:title')),
        dek: normalizeText(jsonLd.description || findMeta(html, 'description') || findMeta(html, 'og:description')),
        author: normalizeText(typeof jsonLd.author === 'string' ? jsonLd.author : jsonLd.author?.name || jsonLd.author?.[0]?.name || ''),
        publishedAt: jsonLd.datePublished || jsonLd.dateModified || '',
        image: imageFromJsonLd(jsonLd.image, url) || absoluteUrl(findMeta(html, 'og:image'), url),
        blocks,
        text,
        method: 'publisher-jsonld',
      };
    }
  }
  ...
  const text = textFromBlocks(blocks);
  return {
    title: normalizeText(findMeta(html, 'og:title') || html.match(/<title\b[^>]*>([\s\S]*?)<\/title>/i)?.[1] || ''),
    dek: normalizeText(findMeta(html, 'description') || findMeta(html, 'og:description')),
    author: normalizeText(findMeta(html, 'author')),
    publishedAt: findMeta(html, 'article:published_time') || '',
    image: absoluteUrl(findMeta(html, 'og:image'), url),
    blocks,
    text,
    method: best ? 'publisher-html' : 'publisher-body',
  };
}

function findAmpUrl(html = '', baseUrl = '') {
  const tag = html.match(/<link\b[^>]*rel=["']amphtml["'][^>]*>/i)?.[0]
    || html.match(/<link\b[^>]*href=["'][^"']+["'][^>]*rel=["']amphtml["'][^>]*>/i)?.[0]
    || '';
...
async function fetchText(url, { timeout = 14000, headers = {} } = {}) {
  ...
  try {
    ...
    return { text: await response.text(), finalUrl: response.url || String(url), contentType: response.headers.get('content-type') || '' };
  } finally {
    clearTimeout(timer);
  }
}
...
function markdownBlocks(markdown = '', baseUrl = '') {
  let body = String(markdown || '').replace(/\r/g, '').trim();
  const marker = body.indexOf('Markdown Content:');
  if (marker >= 0) body = body.slice(marker + 'Markdown Content:'.length).trim();
  body = body
    .replace(/^Title:\s.*$/gmi, '')
  ...
  for (const raw of lines) {
    ...
    if (image) {
      flush();
      const src = absoluteUrl(image[2], baseUrl);
      if (src) blocks.push({ type: 'image', src, alt: image[1] || '' });
      continue;
    }
    const heading = line.match(/^#{1,4}\s+(.+)$/);
    if (heading) { flush(); pushTextBlock(blocks, 'heading', heading[1]); continue; }
    const quote = line.match(/^>\s*(.+)$/);
    if (quote) { flush(); pushTextBlock(blocks, 'quote', quote[1]); continue; }
    const list = line.match(/^[-*+]\s+(.+)$/) || line.match(/^\d+\.\s+(.+)$/);
    if (list) { flush(); pushTextBlock(blocks, 'list', list[1]); continue; }
...
async function fetchJinaReader(url) {
  ...
  const result = await fetchText(readerUrl, {
    ...
    headers: {
      ...
      'X-Remove-Selector': 'header,footer,nav,aside,form,.advertisement,.ads,.related,.comments,.share,.social',
    },
  });
  const blocks = markdownBlocks(result.text, url);
  const text = textFromBlocks(blocks);
  return { blocks, text, method: 'reader-fallback' };
}

function responsePayload({ article, url, publisher }) {
  const text = article.text || textFromBlocks(article.blocks || []);
  ...
  return {
    ...
    text,
    wordCount: count,
    readingMinutes: Math.max(1, Math.ceil(count / 190)),
    full: count >= 60,
    method: article.method || 'unknown',
